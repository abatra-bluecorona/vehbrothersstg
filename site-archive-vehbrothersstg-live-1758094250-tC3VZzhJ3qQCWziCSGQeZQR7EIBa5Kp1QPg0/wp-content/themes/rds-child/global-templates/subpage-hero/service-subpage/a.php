<?php
// Example of setting image sizes for different devices
// ['desktop', 'ipad', 'mobile']
...
<div class="container-fluid service_subpage_banner py-lg-0 py-4">
    <div class="container px-0 py-lg-3 pb-4">
        <div class="row">
            <div class="col-lg-12 py-3 py-lg-4">
                    ...
                    if ($loop->have_posts()) {
                        while ($loop->have_posts()): $loop->the_post();
                            ...
                            if (strtotime($date) >= strtotime(current_time("m/d/Y")) || $noexpiry == 1) {
                                ...
                                <a data-bs-toggle="modal" data-bs-target="#sidebar_request_coupon_form" onclick="couponBannerButtonClick(this);" class="btn btn-secondary mw-226 mt-2">Request Service <i class="icon-chevron-right text_18 line_height_18 ms-2"></i></a>
                                <?php
                            }
                        endwhile;
                        wp_reset_postdata();
                    ...
                    while ($loop->have_posts()): $loop->the_post();
                        ...
                        foreach ($terms as $term):
                            ...
                            if ($get_slug == $term->slug && $i == 0 && get_post_meta(get_the_ID(), "promotion_show_banner_setting", true) == true && (strtotime($date) >= strtotime(current_time("m/d/Y")) || $noexpiry == 1)) { ?>
                                <span class="d-block display1 coupon_title"><?php echo get_post_meta(get_the_ID(), "promotion_title1", true); ?></span>
                                <span class="d-block display2 py-5 coupon_subtitle"><?php echo get_post_meta(get_the_ID(), "promotion_heading", true); ?></span>
                                <span class="d-block display2 py-5 coupon_subtitle2"><?php echo get_post_meta(get_the_ID(), "promotion_subheading", true); ?></span>
                                <a data-bs-toggle="modal" data-bs-target="#sidebar_request_coupon_form" onclick="couponBannerButtonClick(this);" class="btn btn-secondary mw-226 mt-2">Request Service <i class="icon-chevron-right text_18 line_height_18 ms-2"></i></a>
                                <?php $i++;
                            }
                        endforeach;
                ...
                if ($i == 0) {
                    // Check if 'heading' and 'subheading' are not empty before displaying them
                    if (!empty($args["page_templates"]["service_subpage"]["banner"]["heading"])) {
                        ?>
                        <span class="d-block display1 pb-3"><?php echo esc_html($args["page_templates"]["service_subpage"]["banner"]["heading"]); ?></span>
                        <?php
                    }
                
                    if (!empty($args["page_templates"]["service_subpage"]["banner"]["subheading"])) {
                        ?>
                        <span class="d-block display2 py-5"><?php echo esc_html($args["page_templates"]["service_subpage"]["banner"]["subheading"]); ?></span>
                        <?php
                    }
                    ...
                    if (!empty($args["page_templates"]["service_subpage"]["banner"]["button_text"]) && !empty($args["page_templates"]["service_subpage"]["banner"]["button_link"])) {
                        ?>
                        <a href="<?php echo esc_url(get_home_url() . $args["page_templates"]["service_subpage"]["banner"]["button_link"]); ?>" class="btn btn-primary mw-237 mt-3">
                            <?php echo esc_html($args["page_templates"]["service_subpage"]["banner"]["button_text"]); ?>
                            <i class="icon-chevron-right1 ms-1 text_18 line_height_18 d-lg-inline-block"></i>
                        </a>
                        <?php
                    }
                }
